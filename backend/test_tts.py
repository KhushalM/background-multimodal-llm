...
async def test_tts_service():
    ...
    try:
        ...
        for i, test_case in enumerate(test_texts):
            ...
            efficiency = response.duration / response.processing_time if response.processing_time > 0 else 0
        ...
        print(f"   Average efficiency: {total_audio_duration/total_processing_time:.1f}x real-time")
        ...
        batch_responses = await tts_service.synthesize_batch(batch_texts, "batch_test_session")
        
        print(f"✅ Batch processing completed:")
        for i, response in enumerate(batch_responses):
            print(f"   Sentence {i+1}: {response.duration:.2f}s audio, {response.processing_time:.2f}s processing")
        ...
        for voice in supported_voices[:2]:  # Test first 2 voices
            ...
            print(f"   Generated {voice_response.duration:.2f}s audio with {voice} voice")
...
async def test_conversation_tts():
    ...
    try:
        ...
        for i, response_text in enumerate(ai_responses):
            ...
            print(f"🔊 Generated {tts_response.duration:.1f}s audio in {tts_response.processing_time:.1f}s")
