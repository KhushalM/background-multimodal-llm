...
async def test_pipeline_changes():
    ...
    try:
        async with await create_stt_service() as stt_service:
            ...
            print(f"📝 STT Test Result: '{result.text}' (processing time: {result.processing_time:.2f}s)")
    ...
    try:
        async with await create_tts_service() as tts_service:
            ...
            print(f"🔊 TTS Test Result: {response.duration:.2f}s audio generated in {response.processing_time:.2f}s")
